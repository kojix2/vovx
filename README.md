...
標準入力が空の場合はクリップボードの文字列を読み上げます。標準入力もクリップボードも空の場合は終了します。起動後は GUI で声、速度、再生、停止を操作できます。
macOS の「サービス」>「アクション」から呼び出す想定のため、コマンドラインオプションではなく GUI で操作する作りにしています。
既に GUI が起動している場合、次回以降の実行は新しい GUI を増やさず、標準入力のテキストを既存 GUI に渡して再生します。再生中の場合は現在の再生を停止し、新しい入力を再生します。

既存 GUI との連携には UNIX domain socket を使います。通常は `/tmp/vovx-$USER.sock` を使いますが、必要なら `VOVX_SOCKET` で変更できます。
